...
class MLP:
    ...
    def load_data(self, filepath, smiles_col, y_cols, sep=' '):
        data = pd.read_csv(filepath, sep=sep).head(20000) ####### remove head eventually
    ...
    def fingerprint(self):
        ...
        if train_frac is not None:
            self.x_train, self.x_test, self.y_train, self.y_test =
                train_test_split(x, self.y, test_size=test_frac)
        else:
            self.x_train = x
    ...
    def train(self, learning_rate=0.1, epochs=10, batch_size=1, loss='mean_absolute_error', optimizer='sgd'):
